...
public class ItemController {
    ...
    public String itemNew(@Valid ItemFormDto itemFormDto, BindingResult bindingResult,
        ...
        //상품 등록 시 필수 값이 없다면 다시 상품 등록 페이지로 젖ㄴ환
        if (bindingResult.hasErrors()) {
            return "item/itemForm";
        }
        //상품 등록 시 첫 번째 이미지가 없다면에러 메시지와 함께 상품 등록 페이지로 전환.
        //상품의 첫 번째 이미지는 메인 페이지에서 보여줄 상품 이미지로 사용하기 위해 필수값으로 지정
        ...
        try {
            // 상품 저장 로직 호출. 매개변수로 상품 정보와 상품 이미지 정보를 담고 있는 itemImgFileList
            //를 넘겨준다.
    ...
    public String itemManage(ItemSearchDto itemSearchDto,
        ...
        //첫번째 파라미터로는 조회할 페이지 번호, 두번째 파라미터로는 한 번에 가지고 올 데이터 수를 넣어준다.
        //URL 경로에 페이지 번호가 있으면 해당 페이지를 조회하도록 세팅하고, 페이지 번호가 없음녀 0페이지 조회
        ...
        Page<Item> items =
                ...
                //상품 관리 메뉴 하단에 보여줄 페이지 번호의 최대 개수. 5로 설정 했기에 최대 5개의 이동할
                //페이지 번호만 보여준다.
